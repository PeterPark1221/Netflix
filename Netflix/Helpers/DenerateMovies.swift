...
func generateMovies(_ count: Int) -> [Movie] {
    ...
    for _ in 0..<count {
        ...
        let defEpiInfo = CurrentEpisodesInfo(episodeName: epiname, description: epidesc, season: 1, episode: 1)
        
        let allTrailers = generateTrailers(3)
        
        let moreLikeThis = exampleMovies
        
        let movie = Movie(id: id, name: name, thumbnailURL: thumbnail, categories: categories, genre: .AllGenre, year: year, rating: "PG-13", numberOfSeason: numSeasons, currentEpisode: defEpiInfo, defaultEpisodeInfo: defEpiInfo, creator: creators, cast: cast, moreLikeThis: moreLikeThis, episodes: allEpisodes, promotionHeadline: headline, trailers: allTrailers)
...
func generateTrailers(_ count: Int) -> [Trailers] {
    var allTrailers: [Trailers] = []
    ...
    for _ in 0..<count {
        ...
        let trailer = Trailers(id: id, name: name, videoURL: exampleVideoURL, thumbnailImageURL: thumbnail)
...
func generateEpisodes(_ count: Int, forSeason season: Int) -> [Episodes] {
    var allEpisodes: [Episodes] = []
    ...
    for num in 0..<count {
        ...
        let episode = Episodes(id: id, name: name, season: season, episodeNumber: num + 1, thumbnailImageURLString: thumbnail, description: desc, length: randLength, videoURL: exampleVideoURL)
